...
<head>
    ...
    <title>{{ $office->office_name }} Schedule</title>
...
<body>
    <div class="container-fluid mt-4">
        ...
        <div class="text-center mb-4">
            ...
            <h4><b>TOP SCHEDULE FOR <span style="text-transform:uppercase">{{ $office->office_name }}</span></b></h4>
        ...
        <table class="table table-bordered">
            ...
            <thead class="center1">
                ...
                @foreach($top_rpcppe as $rpcppe)
                <th class="text-center py-5">QTY</th>
                <th class="text-center">
                    <p>Total Cost</p>
                    <p><u>{{ $rpcppe->ppe_code }}</u></p>
                    <p>{{ $rpcppe->ppe_name }}</p>
                </th>
                @endforeach
            </thead>
            <tbody>
                <tr>
                    ...
                    <td class="text-center p-4"><b><span
                                style="text-transform:uppercase">{{ $office->office_name }}</span></b></td>
                    @foreach($top_rpcppe as $rpcppe)
                    <td class="text-center p-4">{{ $rpcppe->total_qty }}</td>
                    <td class="text-center p-4">{{ number_format($rpcppe->grand_total, 2) }}</td>
                    @endforeach
                </tr>
            </tbody>
            <tfoot>
